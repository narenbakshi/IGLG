...
TEST(zeromqsubscribe, PositiveNos) { 

    char* pubdata;
    unsigned int microsecond = 1000000;
    
    char *url = "ipc:///tmp/pubsub.ipc";
    testhandlerzmqsub = (handstruct *)malloc(sizeof(Handlerstruct));

    testhandlerzmqsub = chooseSubscribeHandler("ZEROMQ",url,"Tracer",&msgarrvd_test,"Client");
    
    testhandlerzmqsub->framworkptr->subscribe(testhandlerzmqsub);
